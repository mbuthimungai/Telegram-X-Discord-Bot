from APICalls.amazon.tools.tools import TryExcept, Response, yaml_load, userAgents, domain
...
class Amazon:
    ...
    def __init__(self, userInput):
        ...
        self.scrape = yaml_load('selectors')
    ...
    async def dataByAsin(self):
        ...
        """
        if not 'amazon.com' in self.userInput:
            link_to_product = f"https://www.amazon.com/dp/{self.userInput}"
        else:
            link_to_product = self.userInput
        self.userInput = link_to_product
    ...
    async def product_types(self):
        ...
        # Step 3: Find the parent div with the specified id
        parent_div = soup.find('div', id='twister_feature_div')

        # Step 4: Find all elements with the ASIN attribute within this div
        if parent_div:
            asin_elements = parent_div.find_all(attrs={"data-csa-c-item-id": True})
        else:
            asin_elements = []

        # Step 5: Extract the ASINs from these elements
        asins = [elem['data-csa-c-item-id'] for elem in asin_elements]
